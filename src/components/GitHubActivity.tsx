    ...
    >
      <Card className='bg-card/30 relative w-full max-w-7xl overflow-hidden border backdrop-blur-sm'>
        <div className='pointer-events-none absolute inset-0 opacity-5'>
          <div className='border-primary/30 absolute -top-12 -right-12 h-40 w-40 md:h-60 md:w-60 lg:h-80 lg:w-80 rounded-full border' />
          <div className='border-primary/20 absolute top-20 -right-8 h-24 w-24 md:h-32 md:w-32 lg:h-40 lg:w-40 rounded-full border' />
          <div className='border-primary/20 absolute -bottom-20 -left-20 h-60 w-60 md:h-80 md:w-80 lg:h-96 lg:w-96 rounded-full border' />
        </div>

        <CardContent className='w-full p-4 md:p-6 lg:p-8' ref={containerRef}>
          <div className='mx-auto flex w-full max-w-[1200px] flex-col'>
            <div className='mb-6 md:mb-8 flex w-full items-center justify-between'>
              ...
              <Button
                ...
                disabled={isLoading}
                className='h-9 md:h-10 lg:h-11 px-3 md:px-4 text-sm md:text-base'
              >
                <TbRefresh
            ...
            {isLoading ? (
              <Skeleton className='h-[160px] md:h-[200px] lg:h-[240px] w-full' />
            ) : contributionData ? (
              <div className='flex flex-col items-center'>
                <div className='flex'>
                  <div className='text-muted-foreground mr-3 md:mr-4 flex shrink-0 flex-col pt-6 text-xs md:text-sm lg:text-base'>
                  ...
                  >
                    <div
                      className='text-muted-foreground relative mb-1 md:mb-2 flex text-xs md:text-sm lg:text-base'
                      style={{
                    ...
                    <div
                      className='relative mt-6 md:mt-8 flex'
                    ...
                    >
                      ...
                      {showTooltip && (
                        ...
                        >
                          <div className='border-border bg-popover/95 rounded-lg border px-2 py-1 md:px-3 md:py-2 text-xs md:text-sm lg:text-base whitespace-nowrap shadow-md'>
                            {tooltipContent}
                          </div>
                        </div>
                      )}
                ...
                {showLegend && (
                  <div className='text-muted-foreground mt-6 md:mt-8 lg:mt-10 flex items-center gap-2 md:gap-3 text-xs md:text-sm lg:text-base'>
                    <span>Less</span>
                    <div className='flex' style={{ gap: `${Math.max(3, squareGap - 1)}px` }}>
                      {[0, 1, 4, 7, 10].map((count, index) => (
                        <div
                          ...
                          style={{
                            ...
                            backgroundColor: getContributionColor(count, colorScheme),
                          }}
                        />
                      ))}
                    </div>
              ...
              <div className='text-muted-foreground flex h-[160px] md:h-[200px] lg:h-[240px] items-center justify-center text-sm md:text-base lg:text-lg'>
